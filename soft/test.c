...
void interrupt_handler()
{
	...
	if (state == INIT) {
        ...
        if (posx + vecx >= 0 && posx + vecx <= 11) {
            ...
            if (posx == 0) {life1--; vecx = 1;}
            if (posx == 11) {life2--; vecx = -1;}
			posx += vecx;
		}
...
void main()
{
	state = START; posx = 2; posy = 4; p1_pos = 4; p2_pos = 4;
	...
	while (1) {
		...
		if (state == INIT) {
            ...
            *ptr = 2;
            ...
            state = INIT; posx = 2; posy = 4; p1_pos = 4; p2_pos = 4;
	        vecx = 1; vecy = 1; point1 = 0; point2 = 0; life1 = 4; life2 = 4;
            angle1 = 1; angle2 = -1, mode = 0;
		}  
	}
...
void play()
{
	...
	while (life1 != 0 && life2 != 0) {
		...
		if (*rte_ptr1 & mask) {
            ...
            if (posx == 1 && posy >= p1_pos - thr && posy <= p1_pos + thr && vecx < 0) {
		...
		if (*rte_ptr2 & mask) {
			//*led_ptr = (*rte_ptr2 & mask) >> 1;
			if (posx == 10 && posy >= p2_pos - thr && posy <= p2_pos + thr && vecx > 0) {
...
void show_ball(int posx, int posy)
{
    ...
    lcd_putc_with_color(0, 0, (point1 / 10) + '0', 255, 64, 64);
    lcd_putc_with_color(0, 1, (point1 % 10) + '0', 255, 64, 64);
    lcd_putc_with_color(0, 10, (point2 / 10) + '0', 64, 64, 255);
...
{
    lcd_clear_vbuf();
    lcd_putc_with_color(p1pos, 1, '#', 255, 64, 64);
    lcd_putc_with_color(p2pos, 10, '"', 64, 64, 255);
    int x = 8 * 1 + 4;
    int y = 8 * p1pos + 3;
    int length = 11;
